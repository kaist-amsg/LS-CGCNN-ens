...
def main():
	...
	param_path = './COparams/'
	data_path='./example/labeled_data.json'
	...
	atom_init_path='./COparams/atom_init.json'
	max_num_nbr = 9
	radius = 12
	dmin = 0
	step = 0.2
...
#	resume_path = model_name
	...
	with open('use.csv', 'w') as f:
		writer = csv.writer(f)
		header = ['id', 'ensemble'] + ['model%i'%i for i in range(1,ncv+1)]
		writer.writerow(header)
		for idx, tar, pred_ensemble,pred in zip(ids,Y,Ypensemble,Yp):
			row = [idx,pred_ensemble[0]] + [pred[i] for i in range(ncv)]
			writer.writerow(row)
