...
class CIFData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        atoms = read(os.path.join(self.root_dir,cif_id))
...
class JSONData(Dataset):
    def __init__(self, data_path,atom_init_path, max_num_nbr=9, radius=12, dmin=0, step=0.2,
                 random_seed=123):
        self.max_num_nbr, self.radius = max_num_nbr, radius
    ...
    def __getitem__(self, idx):
        ...
        datum = self.data[idx]
        target = datum['prop']
        atoms = Atoms(positions=datum['positions'],numbers=datum['numbers'],cell=datum['cell'],pbc=datum['pbc'])
        crystal = AseAtomsAdaptor.get_structure(atoms)
        atom_fea = np.vstack([self.ari.get_atom_fea(crystal[i].specie.number)
                              for i in range(len(crystal))])
        atom_fea = torch.Tensor(atom_fea)
...
class PickleData(Dataset): # nbr_fea_idx and nbr_fea is pre-calculated. Uses unstable pickle data. For speed only.
    def __init__(self, data_path,atom_init_path, max_num_nbr=9, radius=12, dmin=0, step=0.2,
                 random_seed=123):
        self.max_num_nbr, self.radius = max_num_nbr, radius
    ...
    def __getitem__(self, idx):
        ...
        datum = self.data[idx]
        target = datum['prop']
        atom_fea = np.vstack([self.ari.get_atom_fea(i) for i in datum['numbers']])
        atom_fea = torch.Tensor(atom_fea)
        nbr_fea_idx = np.array(datum['nbr_fea_idx'])
        nbr_fea = np.array(datum['nbr_fea'])
        nbr_fea = self.gdf.expand(nbr_fea)
        atom_fea = torch.Tensor(atom_fea)
        nbr_fea = torch.Tensor(nbr_fea)
        nbr_fea_idx = torch.LongTensor(nbr_fea_idx)
